   ...
   const success = (userLocation) => {
      ...
      fetch(url)
         ...
         .then((userWeather) => {
            ...
            const userSunriseTime =
               userWeather.daily.data[0].sunriseTime * 1000;
            ...
            if (
               userSunriseTime < userCurrentTime &&
               userCurrentTime < userSunsetTime
            ) {
      ...
      return fetch(url)
         ...
         .then((locationData) => {
            ...
            if (locationData.length >= 1) {
               ...
               getWeather(
                  locationData,
                  store.getState().data.locations.index
               )();
               ...
               store.dispatch(
                  throwCallerError(
                     "No results returned.  Try another search term."
                  )
               );
            }
         })
...
export const getHistoryFromWeatherStation = (
   id,
   name,
   distance,
   totalstations
         ...
         .then((data) => {
            if (data.data.length > 1) {
               if (Number(distance) > maxRaiusKm) {
                  store.dispatch(
                     throwCallerError(
                        "No weather history available within 20km"
                     )
                  );
               } else {
                  ...
                  store.dispatch(
                     receiveWeatherHistory(processedData, id, name, distance)
                  );
               }
            } else {
               ...
               if (index + 1 >= totalstations) {
                  store.dispatch(
                     throwCallerError(
                        `Sorry, weather history and trends not available for this location.  Try searching for a nearby location.`
                     )
                  );
...
export const processWeatherHistoryData = (data) => {
      ...
      dataByMonth.forEach((month, index) => {
         ...
         month.datum.c = {
            "Average Temperature": average(
               filterArrayByNull(
                  extractValues(
                     filterByMonth(data, index + 1),
                     "temperature_mean"
                  )
               )
            ),
            "Average High Temp": average(
               filterArrayByNull(
                  extractValues(
                     filterByMonth(data, index + 1),
            ...
            "Total Rainfall": average(
               filterArrayByNull(
                  extractValues(filterByMonth(data, index + 1), "precipitation")
               )
            ),
         };

         month.datum.f = {
            "Average Temperature": celciusToFerinheight(
               average(
                  filterArrayByNull(
                     extractValues(
                        filterByMonth(data, index + 1),
            ...
            "Average High Temp": celciusToFerinheight(
               average(
                  filterArrayByNull(
                     extractValues(
                        filterByMonth(data, index + 1),
            ...
            "Average Low Temp": celciusToFerinheight(
               average(
                  filterArrayByNull(
                     extractValues(
                        filterByMonth(data, index + 1),
   ...
   function processByType() {
      ...
      dataByMonth.forEach((month) => {
         dataByType.forEach((type) => {
            ...
            type.datum.c[thisMonth] = dataByMonth.filter(
               (month) => month.name === thisMonth
            )[0].datum.c[type.name];
            type.datum.f[thisMonth] = dataByMonth.filter(
               (month) => month.name === thisMonth
            )[0].datum.f[type.name];
         });
      });
   ...
   const filterByMonth = (data, month) =>
      data.filter(
         (entry) =>
            Number(
               entry.month.slice(entry.month.length - 2, entry.month.length)
            ) === month
      );

   const extractValues = (array, subvalue) =>
      array.map((value) => value[subvalue]);
   ...
   const indexOfMinValue = (a) =>
      a.reduce((iMin, x, i, arr) => (x < arr[iMin] ? i : iMin), 0);
   ...
   const nullFilteredMaxArray = (array) =>
      array.map((item) => (item == null ? -Infinity : item));
   const nullFilteredMinArray = (array) =>
      array.map((item) => (item == null ? Infinity : item));
...
export function changeLocation(e) {
   getWeather(
      store.getState().data.locations.data,
